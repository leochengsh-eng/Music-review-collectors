...
from src.report.site import ReportMeta, build_static_site
...
def pipeline(mode, window, delivery="pages"):
    ...
    report_output_dir = "docs/reports" if delivery == "pages" else "outputs/html"
    html_path=render_report(rows, window, report_output_dir)
    ...
    if delivery == "pages":
        recommended_count = len([r for r in rows if (r.get("normalized_score_10") or 0) >= 7 and r.get("status") != "manual_check"])
        manual_count = len([r for r in rows if r.get("status") == "manual_check" or r.get("parse_status") != "success" or r.get("dedupe_status") == "uncertain"])
        highlights = tuple(f"{r.get('artist')} — {r.get('album')}" for r in rows if (r.get("normalized_score_10") or 0) >= 7)[:5]
        build_static_site(ReportMeta(str(window.start), str(window.end), html_path.name, recommended_count, manual_count, highlights))
        logger.info("Static site updated under docs/")
    return html_path

def main():
    p=argparse.ArgumentParser(); p.add_argument("--mode", choices=["weekly","backfill","report-only","test-email","build-site"], required=True); p.add_argument("--delivery", choices=["pages","html"], default=None); p.add_argument("--lookback-days", type=int); p.add_argument("--start-date"); p.add_argument("--end-date")
    args=p.parse_args(); settings=load_settings(); lookback=args.lookback_days or settings.report_lookback_days
    if args.mode == "build-site":
        build_static_site()
        return
    if args.mode == "test-email":
        raise SystemExit("Email delivery is disabled by default; use --delivery pages for publishing.")
    ...
    pipeline(args.mode, window, delivery=args.delivery or settings.delivery_mode)
